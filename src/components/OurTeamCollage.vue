...
onMounted(() => {
  ...
  ;['.grid-item-3', '.grid-item-10', '.grid-item-11', '.grid-item-12'].forEach(
    (person, idx, A) => {
      if (idx !== 0) {
        gsap.fromTo(
          person,
  ...
  ;['.grid-item-2', '.grid-item-4', '.grid-item-6'].forEach(
    (person, idx, A) => {
      if (idx !== 0) {
        gsap.fromTo(
          person,
  ...
  ;['.grid-item-7', '.grid-item-5', '.grid-item-9', '.grid-item-8'].forEach(
    (person, idx, A) => {
      if (idx !== 0) {
        gsap.fromTo(
          person,
...
<template>
  <div class="collage grid grid-cols-3 grid-rows-6">
    <div
      ...
      :key="index"
      :class="['grid-item', team.class]"
      :style="{
        backgroundImage: `url(${team.image})`,
        gridArea: team.name.toLowerCase(),
      }"
    >
      <h3>{{ $t(team.name) }}</h3>
      <p>{{ $t(team.role) }}</p>
