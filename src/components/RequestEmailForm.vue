...
<template>
  ...
  >
    ...
    <div
      class="feedback flex w-[90ch] h-[3rem] items-center justify-between gap-[1rem]"
    >
      <form
        class="form flex h-[3rem] items-center justify-between gap-[1rem] grow"
        @submit.prevent="submitForm"
      >
        <label
          :data-text="$t('invest_CTA_mail')"
          class="form-label relative grow h-[1.5rem] bottom-[-0.51rem]"
        >
          <input
            v-model="email"
        ...
        <button
          type="submit"
          class="email-send w-[7.5rem] h-[2.5rem] text-pro-bold grow-0"
        >
          {{ $t('invest_CTA_send') }}
        </button>
      </form>
