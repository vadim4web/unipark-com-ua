...
const computedPadding = computed(() =>
  y.value <= 860 ? Math.floor(y.value / 7) : Math.floor(860 / 7)
)
...
<template>
  <main class="home flex flex-col items-center">
    ...
    <section
      ref="mainInfo"
      class="main-info text-center flex flex-col items-center"
      :style="{ 'padding-top': `calc(3rem + ${computedPadding}px)` }"
    >
      <h1 class="text-[1.3rem] w-[50ch] font-semibold leading-6">
        {{ $t('header') }}
      </h1>
      <h2 class="text-pro-bold w-[27ch] text-[4.25rem] leading-[120%]">
        {{ $t('header_') }}
      </h2>
      <h3 class="text-[1.3rem] w-[55ch] font-semibold leading-6">
        {{ $t('header__') }}
      </h3>
    </section>
    ...
    <section class="production-overview flex flex-col items-center text-center">
      <h2
        class="text-pro-bold text-[4.5rem] w-[35ch] leading-[133%] whitespace-pre-line"
      >
        {{ $t('main_prod_over') }}
      </h2>
      <h3 class="text-pro-bold text-[10rem] leading-[133%] relative w-[17ch]">
        ...
        <router-link
          to="/production"
          class="flex items-center justify-center absolute details text-[1.5rem] rounded-full w-[10rem] h-[10rem] text-regular tracking-wider"
        >
          {{ $t('details') }}
        </router-link>
      </h3>
